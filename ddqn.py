...
class Agent():
    def __init__(self, gamma, epsilon, lr, n_actions, input_dims, mem_size, batch_size, eps_min=0.1, eps_dec=5e-7, tau=1000, checkpoint_dir='checkpoints'):
        ...
        self.dqn = DDQN(self.lr, self.n_actions, 'dqn',
                        self.input_dims, self.checkpoint_dir)
        self.dqn_target = DDQN(self.lr, self.n_actions,
    ...
    def learn(self):
        ...
        next_q_value = self.dqn_target.forward(states_).gather(
            1, self.dqn.forward(states_).argmax(dim=1, keepdim=True)).detach()
        mask = ~dones
